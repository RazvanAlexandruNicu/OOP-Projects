...
public class mkdir implements Command{
    ...
    private String name;
    private static mkdir single_instance = null;
    int iterate;
    String[] vector;
  ...
  //reference to the light
    ...
    public void execute() {
        ...
        if(analyzePath(this.fullPath) == 1)
        {
            ...
            {
               placeToInsert.mkdir(name); 
            }
            else
            {
                if(placeToInsert.getChild(name).getParent() == root)
                {
                    error.println("mkdir: cannot create directory "+placeToInsert.getChild(name).getParent().getPath()+placeToInsert.getChild(name).getName()+": Node exists");
                }
                else
                {
                    error.println("mkdir: cannot create directory "+placeToInsert.getChild(name).getParent().getPath()+"/"+placeToInsert.getChild(name).getName()+": Node exists");
        ...
        {
            error.println("mkdir: "+parentPath+": No such directory");
        }
    }
